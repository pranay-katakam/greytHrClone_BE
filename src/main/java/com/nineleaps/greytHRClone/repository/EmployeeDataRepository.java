...
//    EmployeeData findByEmail(String email);
    ...
    @Query(value = "SELECT * FROM (select emp_id,name,location,manager_id from employee_data where emp_id=?1 ) a CROSS JOIN (select name as manager_name from employee_data where emp_id=(select manager_id from employee_data where emp_id=?1))  b ",nativeQuery = true)
